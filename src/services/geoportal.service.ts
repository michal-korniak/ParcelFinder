    ...
    return this.http.get(this.baseUrl, { params, responseType: 'text' })
      .pipe(
        map(response => {
          ...
          const filteredDataLines = dataLines.filter(line => line !== '');
          if (filteredDataLines.length===0) {
            throw new Error('No parcel data found');
          }

          for(const dataLine of filteredDataLines){

          }

          return filteredDataLines.map(dataLine=>this.mapDataLineToParcelObject(dataLine));
        })
      );
  }

  private mapDataLineToParcelObject(dataLine: string) {
